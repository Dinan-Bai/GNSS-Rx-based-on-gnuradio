...
class blk(gr.basic_block):
    def __init__(self, sps=4,fine_search_loopN = 6):
        ...
        self.num_channels = 7
        self.Nloop = 0
        self.PRN_ref = [1]*self.num_channels
        self.mode = 0 # 0: cold start # 1: tracking mode
        self.reg = [0+0j]*self.chunk_size
        self.PRN_local = [[0] * self.chunk_size for _ in range(self.num_channels)]
        ...
        if pmt.is_pair(msg):
            ...
            if pmt.symbol_to_string(key) == "PRN_ref" and pmt.is_f32vector(value):
                ...
                for ii in range(self.num_channels):
                    PRN_local = self.generate_prn_code(self.PRN_ref[ii])
                    PRN_local = 2*PRN_local - 1
                    PRN_local = np.repeat(PRN_local, self.sps)
                    self.PRN_local[ii][:] = PRN_local
                # print('local PRN updated')
            else:
            ...
            if self.state == 1:  # register mode (or sleeping mode)
                if self.load_data == 0:
                    ...
                    for ii in range(self.num_channels):
                        ...
                        output_items[self.num_channels+ii][0,:] = self.PRN_local[ii][:]
                    self.load_data = 0
                    return 1
